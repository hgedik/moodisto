...
import { PlayerDockProvider } from '@/components/player/player-dock';
...
export default function VenueConsoleLayout({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {({ user, signOut }) => (
        // The player lives here rather than on its own page: this layout stays mounted while the
        // pages under it change, which is what keeps the music playing during navigation.
        <PlayerDockProvider venueId={user.venue.id}>
